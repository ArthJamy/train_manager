...
import { trainsFR as baseFR } from './trainsFR.js';
import { trainsDE as baseDE } from './trainsDE.js';
...
function validateTrainBeforeSave() {
  ...
  const all = [...state.trainsFR, ...state.trainsDE];
...
function performExport() {
  ...
  const target = state.current.pays === 'DE' ? 'DE' : 'FR';

  // 🧹 Supprime TOUTES les anciennes occurrences de ce train dans les deux fichiers
  removeTrainEverywhere(trainObj.id);

  // 🟢 Ajoute ou remplace dans le fichier cible
  const arrTarget = (target === 'FR') ? state.trainsFR : state.trainsDE;
  ...
  // 💾 Téléchargement simultané des deux fichiers
  download('trainsFR.js', frText);
  download('trainsDE.js', deText);

  renderAllTrajets();
  toast('Export téléchargé (FR & DE).', 'ok');

  // 🔄 Réinitialise le mode édition après export
...
function openPopupTrains() {
  ...
  function renderList() {
    ...
    list.forEach(t => {
      ...
      div.style.background = p === 'FR' ? '#e6f0ff' : p === 'DE' ? '#fff7cc' : '#f9f9f9';
...
function loadTrainData(train) {
  ...
  paysSel.classList.remove('FR', 'DE');
...
function bindActions() {
    ...
    function renderList() {
      ...
      items.forEach(e => {
        ...
        if (sortBy === 'pays') {
          ...
          bg = p === 'FR' ? '#e6f0ff' : p === 'DE' ? '#fff7cc' : '#f9f9f9';
        } else if (sortBy === 'vitesse') {
          bg = getSpeedColor(e.vitesseMax);
  ...
  $('#btn-new-train').addEventListener('click', () => {
    ...
    const trainObj = toTrainObjectForSave();
    const target = state.current.pays === "DE" ? state.trainsDE : state.trainsFR;
...
function renderAllTrajets() {
  ...
  state.current.trajets.forEach((trajet, index) => {
    ...
    `;
    header.innerHTML = `
      <span>${trajet.nom || '(Trajet sans nom)'}</span>
      <span class="muted">${trajet.dessertes.length} gares</span>
    `;
